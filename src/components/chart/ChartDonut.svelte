...
type apexChartObj = { globals: { seriesTotals: number[] } };
      ...
      donut: {
        ...
        labels: {
          ...
          value: {
            show: true,
            fontSize: "20px",
            formatter: (w: number) => `S/ ${Math.trunc(w)}`,
          },
          total: {
            show: true,
            fontWeight: 600,
            fontSize: "20px",
            formatter: (w: apexChartObj) => `S/ ${Math.trunc(apexTotal(w))}`,
          },
        },
      },
    },
...
function apexTotal(w: apexChartObj) {
